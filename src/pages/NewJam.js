...
function NewJam({user, joinroom, joinperformer}){
  ...
  const [password, setpassword] = useState("")
    ...
    <div className="mb-6 w-80 md:w-96 ml-10 mt-20" >
     ...
     <br/><br/> <input className="shadow appearance-none border rounded w-full py-2 px-3  border-yellow-400 bg-transparent text-white leading-tight focus:outline-none focus:shadow-outline" id="passkey" type="text" placeholder="enter passkey (only for performers)"  onChange={e => setpassword(e.target.value)} value={password}/>
    </div>
...
<button className=" ml-10  p-1 w-40 mb-2 rounded-lg bg-yellow-400 text-black" onClick={() => {joinperformer(conferenceAlias, password)} }>join as performer</button>
